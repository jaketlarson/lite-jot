class SupportTicketNotifier < ActionMailer::Base
  ...
  def send_creation_email(ticket, user)
    @ticket = ticket
    @user = user
  ...
  def send_response_email(ticket, user)
    @ticket = ticket
    @user = user
    m = mail( :to => @user.email,
  ...
  def send_creation_admin_notification_email(ticket, user)
    @ticket = ticket
    @user = user
  ...
  def send_response_admin_notification_email(ticket, user)
    @ticket = ticket
    @user = user
    m = mail( :to => Rails.application.secrets.from_email_address,
