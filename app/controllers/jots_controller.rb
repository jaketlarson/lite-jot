class JotsController < ApplicationController
  ...
  def create
    ...
    jots.each do |jot|
      # Folder and topic will be autogenerated if not yet created
      # Either way, we need the info..
      get_folder = Folder.autogenerate_if_nesessary(jot, current_user)
      new_folder = get_folder[:new_folder]
      folder_id = get_folder[:folder_id]

      get_topic = Topic.autogenerate_if_nesessary(jot, current_user)
      new_topic = get_topic[:new_topic]
      ...
      folder_is_owned = Folder.is_owned(jot.folder_id, current_user)

      if !folder_is_owned && Folder.is_empty(jot.folder_id)
      ...
      if jot.jot_type == 'checklist'
        jot.content = Jot.append_id_to_new_checklist(jot.content)
      end
      ...
      if params['jot_type'] == 'checklist'
        ...
        new_version.each do |new_item|
          ...
          if new_item['id'].length == 0
            new_item['id'] = generate_checklist_item_id
          else
